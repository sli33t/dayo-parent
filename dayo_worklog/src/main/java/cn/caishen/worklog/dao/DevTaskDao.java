...
public interface DevTaskDao extends BaseMapper<DevTask> {
    ...
    @Select("<script>" +
            ...
            "TB_FEEDBACK.STATUS, TB_FEEDBACK.FINISHED, TB_FEEDBACK.FINISH_TIME, TB_DEVTASK.FINISHED AS DEV_FINISHED, " +
            ...
            "LEFT JOIN TB_DEVTASK ON TB_FEEDBACK.FEEDBACK_ID = TB_DEVTASK.FEEDBACK_ID AND COALESCE(TB_DEVTASK.FINISHED, 0) != 2 " +
    ...
    @Select("SELECT TB_FEEDBACK.FEEDBACK_ID, TB_DEVTASK.DEVTASK_ID, TB_FEEDBACK.REQUIRE_DATE, TB_FEEDBACK.PROBLEM_TEXT, TB_CUSTOMER.CUSTOMER_NAME, " +
            "TB_DEVTASK.TASK_TEXT, TB_USER.USER_NAME, TB_VERSION.VERSION_NAME " +
            "FROM TB_FEEDBACK " +
            "INNER JOIN TB_CUSTOMER ON TB_FEEDBACK.CUSTOMER_ID = TB_CUSTOMER.CUSTOMER_ID " +
